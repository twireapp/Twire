...
abstract class MainFragment<E> :
    BindingFragment<ActivityMainBinding>(ActivityMainBinding::inflate) {
    ...
    override fun onViewCreated(view: View, savedInstance: Bundle?) {
        ...
        mDrawerFragment = binding.drawerFragment.getFragment<NavigationDrawerFragment>()

        initErrorView()
        initTitleAndIcon()

        val activity: AppCompatActivity = requireActivity() as AppCompatActivity
        activity.setSupportActionBar(mMainToolbar)
        if (activity.supportActionBar != null) activity.supportActionBar!!.title = ""
        mMainToolbar.setPadding(
            0,
            0,
            Service.dpToPixels(requireContext(), 5f),
            0
        ...
        Insetter.builder()
            .setOnApplyInsetsListener { view: View?, windowInsets: WindowInsetsCompat?, initialState: ViewState? ->
                ...
                binding.mainDecorativeToolbar.setMinimumHeight(
                    resources.getDimension(R.dimen.additional_toolbar_height)
                        .toInt() + insets.top
                )
        ...
        view.viewTreeObserver.addOnWindowFocusChangeListener(this::onWindowFocusChanged)
    }
    ...
    private fun checkForTip() {
        if (!isTipsShown) {
            try {
                mTooltip = TooltipWindow(requireActivity(), TooltipWindow.POSITION_TO_RIGHT)
    ...
    fun initActivityAnimation() {
        val intent = this.arguments ?: bundleOf()
        fromToolbarPosition = intent.getFloat(
            getString(R.string.decorative_toolbar_position_y), -1f
        )

        fromMainToolbarPosition = intent.getFloat(
     ...
     */
    fun transitionToOtherMainActivity(
        aIntent: Class<out Fragment>,
        extras: Bundle? = null
    ) {
        hideErrorView()
        ...
        val animationListener: Animation.AnimationListener = object : AnimationListenerAdapter() {
            override fun onAnimationEnd(animation: Animation?) {
                ...
                navigate(
                    aIntent,
                    bundleOf(
                        getString(R.string.decorative_toolbar_position_y) to mDecorativeToolbar.translationY,
                        getString(R.string.main_toolbar_position_y) to mMainToolbar.translationY
                    ).apply { if (extras != null) putAll(extras) },
                    single = true
                )
            }
        }
